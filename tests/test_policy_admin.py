...
def test_admin_audit_logs_combines_policy_name_and_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="policy-name-version-ip",
                country_code="EE",
                request_type="access",
                allowed=True,
                risk_score=0,
                ...
                risk_signals="",
                policy_matched=True,
                policy_name="named-version-policy",
                policy_version=1,
                workflow_version=None,
                trace_id="policy-name-version-trace",
                idempotency_key="policy-name-version-idem",
                request_fingerprint="policy-name-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "policy-name-version-trace" in trace_ids
    assert all(item["policy_name"] == "named-version-policy" for item in response.json())
    assert all(item["policy_version"] == 1 for item in response.json())


def test_admin_audit_logs_policy_name_and_wrong_version_returns_empty(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="policy-name-wrong-version-ip",
                country_code="EE",
                request_type="access",
                allowed=True,
                risk_score=0,
                ...
                risk_signals="",
                policy_matched=True,
                policy_name="wrong-version-policy",
                policy_version=2,
                workflow_version=None,
                trace_id="policy-name-wrong-version-trace",
                idempotency_key="policy-name-wrong-version-idem",
                request_fingerprint="policy-name-wrong-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "policy-name-wrong-version-trace" not in trace_ids


def test_admin_audit_logs_combines_policy_name_and_workflow_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="policy-name-workflow-ip",
                country_code="EE",
                request_type="access",
                allowed=True,
                risk_score=0,
                ...
                risk_signals="",
                policy_matched=True,
                policy_name="named-workflow-policy",
                policy_version=1,
                workflow_version=1,
                trace_id="policy-name-workflow-trace",
                idempotency_key="policy-name-workflow-idem",
                request_fingerprint="policy-name-workflow-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "policy-name-workflow-trace" in trace_ids
    assert all(item["policy_name"] == "named-workflow-policy" for item in response.json())
    assert all(item["workflow_version"] == 1 for item in response.json())


def test_admin_audit_logs_combines_policy_name_and_workflow_configured(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="policy-name-configured-ip",
                country_code="EE",
                request_type="access",
                allowed=True,
                risk_score=0,
                ...
                risk_signals="",
                policy_matched=True,
                policy_name="named-configured-policy",
                policy_version=1,
                workflow_version=1,
                trace_id="policy-name-configured-trace",
                idempotency_key="policy-name-configured-idem",
                request_fingerprint="policy-name-configured-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "policy-name-configured-trace" in trace_ids
    assert all(item["policy_name"] == "named-configured-policy" for item in response.json())
    assert all(item["workflow_version"] is not None for item in response.json())


def test_admin_audit_logs_and_count_agree_for_policy_name_version_and_workflow(client):
    ensure_setup(client)
...
def test_admin_audit_logs_combines_allowed_and_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="allowed-policy-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                risk_signals="failure_burst",
                policy_matched=True,
                policy_name="allowed-version-policy",
                policy_version=1,
                workflow_version=None,
                trace_id="allowed-policy-version-trace",
                idempotency_key="allowed-policy-version-idem",
                request_fingerprint="allowed-policy-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "allowed-policy-version-trace" in trace_ids
    assert all(item["allowed"] is False for item in response.json())
    assert all(item["policy_version"] == 1 for item in response.json())


def test_admin_audit_logs_allowed_excludes_wrong_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="allowed-wrong-policy-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                policy_version=2,
                workflow_version=None,
                trace_id="allowed-wrong-policy-version-trace",
                idempotency_key="allowed-wrong-policy-version-idem",
                request_fingerprint="allowed-wrong-policy-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "allowed-wrong-policy-version-trace" not in trace_ids


def test_admin_audit_logs_combines_allowed_and_workflow_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="allowed-workflow-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                policy_version=None,
                workflow_version=1,
                trace_id="allowed-workflow-version-trace",
                idempotency_key="allowed-workflow-version-idem",
                request_fingerprint="allowed-workflow-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "allowed-workflow-version-trace" in trace_ids
    assert all(item["allowed"] is False for item in response.json())
    assert all(item["workflow_version"] == 1 for item in response.json())


def test_admin_audit_logs_combines_allowed_and_workflow_configured_false(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="allowed-default-workflow-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="allowed-default-workflow-trace",
                idempotency_key="allowed-default-workflow-idem",
                request_fingerprint="allowed-default-workflow-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "allowed-default-workflow-trace" in trace_ids
    assert all(item["allowed"] is False for item in response.json())
    assert all(item["workflow_version"] is None for item in response.json())


def test_admin_audit_logs_and_count_agree_for_allowed_policy_and_workflow_filters(client):
    ensure_setup(client)
...
def test_admin_audit_logs_combines_risk_signal_and_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="signal-policy-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                risk_signals="failure_burst,new_ip",
                policy_matched=True,
                policy_name="signal-version-policy",
                policy_version=1,
                workflow_version=None,
                trace_id="signal-policy-version-trace",
                idempotency_key="signal-policy-version-idem",
                request_fingerprint="signal-policy-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "signal-policy-version-trace" in trace_ids
    assert all(item["policy_version"] == 1 for item in response.json())


def test_admin_audit_logs_risk_signal_excludes_wrong_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="signal-wrong-policy-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                risk_signals="failure_burst",
                policy_matched=True,
                policy_name="signal-wrong-version-policy",
                policy_version=2,
                workflow_version=None,
                trace_id="signal-wrong-policy-version-trace",
                idempotency_key="signal-wrong-policy-version-idem",
                request_fingerprint="signal-wrong-policy-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "signal-wrong-policy-version-trace" not in trace_ids


def test_admin_audit_logs_combines_risk_signal_and_workflow_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="signal-workflow-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                policy_version=None,
                workflow_version=1,
                trace_id="signal-workflow-version-trace",
                idempotency_key="signal-workflow-version-idem",
                request_fingerprint="signal-workflow-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "signal-workflow-version-trace" in trace_ids
    assert all(item["workflow_version"] == 1 for item in response.json())


def test_admin_audit_logs_combines_risk_signal_and_workflow_configured_false(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="signal-default-workflow-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="signal-default-workflow-trace",
                idempotency_key="signal-default-workflow-idem",
                request_fingerprint="signal-default-workflow-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "signal-default-workflow-trace" in trace_ids
    assert all(item["workflow_version"] is None for item in response.json())


def test_admin_audit_logs_and_count_agree_for_risk_signal_policy_and_workflow_filters(client):
    ensure_setup(client)
...
def test_admin_audit_logs_combines_min_risk_score_and_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="risk-policy-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                risk_signals="failure_burst",
                policy_matched=True,
                policy_name="risk-version-policy",
                policy_version=1,
                workflow_version=None,
                trace_id="risk-policy-version-trace",
                idempotency_key="risk-policy-version-idem",
                request_fingerprint="risk-policy-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "risk-policy-version-trace" in trace_ids
    assert all(item["risk_score"] >= 80 for item in response.json())
    assert all(item["policy_version"] == 1 for item in response.json())


def test_admin_audit_logs_min_risk_score_excludes_lower_score_with_matching_policy_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="low-risk-policy-version-ip",
                country_code="EE",
                request_type="access",
                allowed=True,
                risk_score=79,
                ...
                risk_signals="",
                policy_matched=True,
                policy_name="low-risk-version-policy",
                policy_version=1,
                workflow_version=None,
                trace_id="low-risk-policy-version-trace",
                idempotency_key="low-risk-policy-version-idem",
                request_fingerprint="low-risk-policy-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "low-risk-policy-version-trace" not in trace_ids


def test_admin_audit_logs_combines_min_risk_score_and_workflow_version(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="risk-workflow-version-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=90,
                ...
                policy_version=None,
                workflow_version=1,
                trace_id="risk-workflow-version-trace",
                idempotency_key="risk-workflow-version-idem",
                request_fingerprint="risk-workflow-version-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "risk-workflow-version-trace" in trace_ids
    assert all(item["risk_score"] >= 90 for item in response.json())
    assert all(item["workflow_version"] == 1 for item in response.json())


def test_admin_audit_logs_combines_min_risk_score_and_workflow_configured_false(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="risk-default-workflow-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=85,
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="risk-default-workflow-trace",
                idempotency_key="risk-default-workflow-idem",
                request_fingerprint="risk-default-workflow-fingerprint",
                user_agent="pytest",
                decision_version="test",
            )
        )
    ...
    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "risk-default-workflow-trace" in trace_ids
    assert all(item["risk_score"] >= 85 for item in response.json())
    assert all(item["workflow_version"] is None for item in response.json())


def test_admin_audit_logs_and_count_agree_for_min_risk_policy_and_workflow_filters(client):
    ensure_setup(client)
...
def test_admin_audit_logs_combines_time_range_and_allowed(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="time-allowed-trace",
                idempotency_key="time-allowed-idem",
                request_fingerprint="time-allowed-fingerprint",
                user_agent="pytest",
                decision_version="test",
                created_at=datetime(2030, 2, 1, 12, 0, 0),
            )
        )
        db.commit()
    ...
    response = client.get(
        ...
        params={
            "tenant_id": "tenant-demo",
            "allowed": False,
            "from_time": "2030-02-01T11:00:00",
            "to_time": "2030-02-01T13:00:00",
        },
    )

    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "time-allowed-trace" in trace_ids
    assert all(item["allowed"] is False for item in response.json())


def test_admin_audit_logs_time_range_and_allowed_excludes_wrong_decision(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                device_id="device-A1",
                user_id="user-123",
                ip_hash="time-allowed-mismatch-ip",
                country_code="EE",
                request_type="access",
                allowed=False,
                risk_score=80,
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="time-allowed-mismatch-trace",
                idempotency_key="time-allowed-mismatch-idem",
                request_fingerprint="time-allowed-mismatch-fingerprint",
                user_agent="pytest",
                decision_version="test",
                created_at=datetime(2030, 2, 2, 12, 0, 0),
            )
        )
        db.commit()
    ...
    response = client.get(
        ...
        params={
            "tenant_id": "tenant-demo",
            "allowed": True,
            "from_time": "2030-02-02T11:00:00",
            "to_time": "2030-02-02T13:00:00",
        },
    )

    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "time-allowed-mismatch-trace" not in trace_ids


def test_admin_audit_logs_combines_time_range_and_min_risk_score(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="time-min-risk-trace",
                idempotency_key="time-min-risk-idem",
                request_fingerprint="time-min-risk-fingerprint",
                user_agent="pytest",
                decision_version="test",
                created_at=datetime(2030, 2, 3, 12, 0, 0),
            )
        )
        db.commit()
    ...
    response = client.get(
        ...
        params={
            "tenant_id": "tenant-demo",
            "min_risk_score": 75,
            "from_time": "2030-02-03T11:00:00",
            "to_time": "2030-02-03T13:00:00",
        },
    )

    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "time-min-risk-trace" in trace_ids
    assert all(item["risk_score"] >= 75 for item in response.json())


def test_admin_audit_logs_time_range_min_risk_score_excludes_lower_score(client):
    ensure_setup(client)
    ...
    with SessionLocal() as db:
        db.add(
            RequestLog(
                ...
                policy_version=None,
                workflow_version=None,
                trace_id="time-min-risk-low-trace",
                idempotency_key="time-min-risk-low-idem",
                request_fingerprint="time-min-risk-low-fingerprint",
                user_agent="pytest",
                decision_version="test",
                created_at=datetime(2030, 2, 4, 12, 0, 0),
            )
        )
        db.commit()
    ...
    response = client.get(
        ...
        params={
            "tenant_id": "tenant-demo",
            "min_risk_score": 75,
            "from_time": "2030-02-04T11:00:00",
            "to_time": "2030-02-04T13:00:00",
        },
    )

    assert response.status_code == 200
    trace_ids = {item["trace_id"] for item in response.json()}
    assert "time-min-risk-low-trace" not in trace_ids


def test_admin_audit_logs_and_count_agree_for_time_allowed_and_min_risk_score(client):
    ensure_setup(client)
    ...
    params = {
        ...
        "allowed": False,
        "min_risk_score": 101,
        "from_time": "2030-02-01T00:00:00",
        "to_time": "2030-02-05T00:00:00",
    }
...
def test_admin_audit_logs_risk_signal_same_time_orders_by_id_desc(client):
    ensure_setup(client)

    same_time = datetime(2030, 5, 1, 12, 0, 0)
    ...
    with SessionLocal() as db:
        ...
        for i in range(3):
            log = RequestLog(
                ...
                policy_version=None,
                workflow_version=None,
                trace_id=f"signal-order-trace-{i}",
                idempotency_key=f"signal-order-idem-{i}",
                request_fingerprint=f"signal-order-fingerprint-{i}",
                user_agent="pytest",
                decision_version="test",
                created_at=same_time,
            )
    ...
    trace_ids = [
        item["trace_id"]
        for item in response.json()
        if item["trace_id"].startswith("signal-order-trace-")
    ]
    assert trace_ids == expected


def test_admin_audit_logs_risk_signal_same_time_offset_uses_id_tiebreak(client):
    ensure_setup(client)

    same_time = datetime(2030, 5, 2, 12, 0, 0)
    ...
    with SessionLocal() as db:
        ...
        for i in range(3):
            log = RequestLog(
                ...
                policy_version=None,
                workflow_version=None,
                trace_id=f"signal-offset-trace-{i}",
                idempotency_key=f"signal-offset-idem-{i}",
                request_fingerprint=f"signal-offset-fingerprint-{i}",
                user_agent="pytest",
                decision_version="test",
                created_at=same_time,
            )
...
def test_admin_audit_logs_risk_signal_exact_filter_excludes_longer_token_during_pagination(client):
    ensure_setup(client)

    same_time = datetime(2030, 5, 3, 12, 0, 0)
        ...
        db.add_all(
            [
                RequestLog(
                    ...
                    policy_version=None,
                    workflow_version=None,
                    trace_id="signal-exact-trace",
                    idempotency_key="signal-exact-idem",
                    request_fingerprint="signal-exact-fingerprint",
                    user_agent="pytest",
                    decision_version="test",
                    created_at=same_time,
                ),
                RequestLog(
                    ...
                    policy_version=None,
                    workflow_version=None,
                    trace_id="signal-longer-trace",
                    idempotency_key="signal-longer-idem",
                    request_fingerprint="signal-longer-fingerprint",
                    user_agent="pytest",
                    decision_version="test",
                    created_at=same_time,
                ),
    ...
    assert response.status_code == 200
    assert len(response.json()) == 1
    assert response.json()[0]["trace_id"] == "signal-exact-trace"


def test_admin_audit_logs_risk_signal_exact_middle_token_survives_pagination(client):
    ensure_setup(client)
        ...
        for i in range(3):
            db.add(
                RequestLog(
                    ...
                    policy_version=None,
                    workflow_version=None,
                    trace_id=f"signal-middle-trace-{i}",
                    idempotency_key=f"signal-middle-idem-{i}",
                    request_fingerprint=f"signal-middle-fingerprint-{i}",
                    user_agent="pytest",
                    decision_version="test",
                    created_at=datetime(2030, 5, 4, 12, i, 0),
                )
            )
        db.commit()
    ...
    assert response.status_code == 200
    assert len(response.json()) == 1
    assert response.json()[0]["trace_id"] == "signal-middle-trace-1"


def test_admin_audit_logs_and_count_agree_for_exact_risk_signal_with_pagination(client):
    ensure_setup(client)
        ...
        for i in range(3):
            db.add(
                RequestLog(
                    ...
                    policy_version=None,
                    workflow_version=None,
                    trace_id=f"signal-count-trace-{i}",
                    idempotency_key=f"signal-count-idem-{i}",
                    request_fingerprint=f"signal-count-fingerprint-{i}",
                    user_agent="pytest",
                    decision_version="test",
                    created_at=datetime(2030, 5, 5, 12, i, 0),
                )
            )
        db.commit()
